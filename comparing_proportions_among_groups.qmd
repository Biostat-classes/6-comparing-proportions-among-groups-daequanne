...
```{r}
sex_counts <- table(heart_attacks$SEX)
chisq.test(sex_counts, p = c(0.492, 0.508))
```

Heart attacks are not evenly distributed across genders based on the population split. The result was significant, so we reject the null hypothesis. Males are overrepresented.
...
```{r}
survival_gender <- table(heart_attacks$SEX, heart_attacks$DIED)
chisq.test(survival_gender)
```

Survival is not independent of gender. The test was significant, so gender appears to affect survival after a heart attack.
...
```{r}
under30 <- subset(heart_attacks, AGE < 30)
under30_tab <- table(under30$SEX, under30$DIED)
fisher.test(under30_tab)
```

For people under 30, survival appears to be independent of gender. The result was not significant, but the sample size is very small.
...
rownames(dolphins) <- c("Morning", "Noon", "Afternoon", "Night")
colnames(dolphins) <- c("Traveling", "NotTraveling")
...
Traveling is not independent of time of day. Dolphins travel more at certain times (especially afternoon) and less at others (like morning). Post-hoc tests confirm these differences.
...
rownames(smoking_exercise) <- c("Heavy", "Never", "Occas", "Regul")
colnames(smoking_exercise) <- c("Freq", "None", "Some")

chisq.test(smoking_exercise)
```

Smoking appears to be independent of exercise. The result was not significant, so there’s no clear relationship between the two.
